...
def subscribe_thank_you(request):
    return render(request, "emails/subscribe_thank_you.html")
...
def subscribe(request):
    if request.method == "POST":
        ...
        if email:
            ...
            mailing_list, created = SimpleMailingList.objects.get_or_create(
                email=email)
            if not created:
                # Resubscribe if they were unsubscribed
                mailing_list.subscribed = True
                mailing_list.save()
                # Redirect to the thank-you page
                return redirect("subscribe_thank_you")
            return redirect("subscribe")  # Redirect back on failure
        return render(request, "emails/subscribe.html")
...
# The complicated email signu p we need to work on
# views.py
